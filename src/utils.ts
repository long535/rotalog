import { differenceInMinutes, parseISO, addSeconds, subMinutes, format } from 'date-fns';
...
export function getShiftTotalHours(shift: Shift): number {
...
/** Generate CSV content from shifts and settings — single source of truth for all export paths */
export function generateCSVContent(shifts: Shift[], settings: AppSettings): string {
  const headers = ['Date', 'Start Time', 'End Time', 'Break (min)', 'Paid Hours', 'Is Annual Leave', 'Annual Leave Used (h)', 'Annual Leave Earned (h)', 'Is Sick Leave', 'Sick Leave Hours (h)', 'Is Overtime', 'Hourly Wage', 'Wages', 'Job ID', 'Notes'];
  ...
  return csvContent + settingsJson;
}
