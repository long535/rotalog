export const translations = {
  zh: {
    appTitle: '班次記錄',
    importCSV: '匯入 CSV',
    exportCSV: '導出 CSV',
    settings: '設定',
    filterAll: '全部',
    filterWeek: '週',
    filterMonth: '月',
    filterYear: '年',
    noRecords: '沒有找到班次記錄',
    edit: '編輯',
    duplicate: '複製',
    delete: '刪除',
    notes: '備註',
    total: '總計',
    annualLeave: '年假',
    sickLeave: '病假',
    overtime: '加班',
    periodEarned: '本期獲得',
    periodUsed: '使用',
    leaveBalance: '年假總結餘',
    estimateOnly: '估算值僅供參考',
    addShift: '新增班次',
    editShift: '編輯班次',
    date: '日期',
    days: '天',
    addDay: '新增一天',
    addDayHint: '提示：可以一次新增多天的相同班次',
    startTime: '開始時間',
    endTime: '結束時間',
    breakTime: '休息時間 (分鐘, 無薪)',
    dailyPaidHours: '單日有薪工時',
    earnedLeave: '獲得年假',
    hourlyWage: '時薪',
    notesOptional: '備註 (選填)',
    notesPlaceholder: '輸入備註...',
    annualLeaveLabel: '年假 (Annual Leave)',
    annualLeaveDesc: '將此班次標記為使用年假',
    useHours: '使用時數',
    default: '預設',
    settingsTitle: '設定',
    defaultWage: '預設時薪',
    currency: '貨幣單位',
    defaultBreak: '預設休息時間 (分鐘)',
    theme: '主題',
    themeLight: '淺色 (Light)',
    themeDark: '深色 (Dark)',
    themeColor02: 'Color02 (Gundam)',
    language: '語言 (Language)',
    ukTax: '英國稅務估算 (UK Tax)',
    ukTaxDesc: '自動估算 Tax, N.I., Pension',
    saveSettings: '儲存設定',
    importSuccess: '成功匯入 {count} 筆班次記錄！',
    importError: '無法解析 CSV 檔案，請確認格式是否正確。',
    clickDayHint: '點選星期幾以快速新增同週的班次',
    weekDays: ['日', '一', '二', '三', '四', '五', '六'],
    weekStartsOn: '一週的第一天',
    weekStartsOnOptions: ['週日', '週一', '週二', '週三', '週四', '週五', '週六'],
    listView: '列表',
    calendarView: '月曆',
    hours: '小時',
    addPhoto: '新增照片',
    takePhoto: '拍照',
    chooseFromGallery: '從相簿選擇',
    removePhoto: '移除照片',
    photo: '照片',
    timer: '計時器',
    breakTimer: '午休計時器',
    startTimer: '開始',
    pauseTimer: '暫停',
    resumeTimer: '繼續',
    resetTimer: '重置',
    paused: '已暫停',
    timerEnded: '午休結束',
    timerEndedDesc: '該上班了！',
    timerWarning: '準備打卡',
    timerWarningDesc: '20 秒後午休結束',
    timerPausedDesc: '計時器已暫停',
    minutes: '分鐘',
    min: '分',
    sec: '秒',
    reminders: '提醒',
    remindersDesc: '上班前提醒',
    reminder1h: '1 小時前',
    reminder30m: '30 分鐘前',
    hasReminders: '已設提醒',
    jobs: '工作地點',
    addJob: '新增工作地點',
    editJob: '編輯工作地點',
    jobName: '工作地點名稱',
    jobNamePlaceholder: '輸入工作地點...',
    noJob: '未分類 (Unassigned)',
    assignJob: '指派工作',
    allJobs: '全部工作地點',
    defaultJob: '預設工作地點',
    setAsDefault: '設為預設',
    deleteJob: '刪除工作地點',
    deleteJobConfirm: '確定要刪除此工作地點嗎？',
    jobWage: '時薪',
    jobBreak: '預設休息時間 (分鐘)',
    noJobsYet: '尚未新增工作地點',
    jobColor: '顏色',
    groupBy: '按工作分組',
    liveEarning: '正在賺取',
    excludeBreak: '扣除休息',
    timeLeft: '距離下班',
    projectedTotal: '預計總收入',
    importConflict: '發現 {count} 個重複班次（起止時間完全相同），請選擇：',
    importConflictSkip: '跳過重複',
    importConflictAddAll: '全部新增',
    importConflictOverwrite: '覆蓋重複',
    // Goal Tracker
    monthlyGoal: '月度目標',
    goalHours: '工時目標 (小時)',
    goalEarnings: '收入目標',
    goalProgress: '本月進度',
    goalHit: '🎉 已達標！',
    goalNotSet: '未設定目標',
    setGoal: '設定目標',
    hoursGoal: '工時',
    earningsGoal: '收入',
    // Monthly Report
    monthlyReport: '月結報告',
    generateReport: '生成報告',
    reportTitle: '薪資條',
    reportPeriod: '報告期間',
    reportTotalHours: '總工時',
    reportTotalEarnings: '總收入',
    reportRegularHours: '正常工時',
    reportOvertimeHours: '加班時數',
    reportAnnualLeave: '年假',
    reportSickLeave: '病假',
    reportShiftCount: '班次數',
    reportAvgPerShift: '平均每班',
    shareReport: '分享報告',
    closeReport: '關閉',
    // Recurring Shifts
    recurring: '重複排班',
    recurringNone: '不重複',
    recurringDaily: '每天',
    recurringWeekly: '每週',
    recurringCustom: '自訂日期',
    recurringDays: '選擇星期',
    recurringUntil: '重複至',
    recurringPreview: '預覽班次',
    recurringCount: '共 {count} 個班次',
    recurringSelectDays: '請選擇至少一天',
    // Paid Breaks
    paidBreaks: '帶薪 Break',
    addPaidBreak: '+ 加入咖啡 Break',
    paidBreakDuration: '每次時長（分鐘）',
    paidBreakCount: '次數',
    paidBreakReminder: '開始/結束提醒',
    paidBreakStart: '☕ 咖啡 Break',
    paidBreakReturn: '⏰ 返回工作',
    paidBreakLabel: '帶薪 Break ×{count}（每次 {min} 分鐘）',
    paidBreakTotal: '帶薪 Break 合計',
    removePaidBreak: '移除',
    coffeeBreak: '☕ 咖啡 Break',
    startCoffeeBreak: '開始 {min} 分鐘 Break',
    noPaidBreak: '未設定帶薪 Break',
    // Overlap
    overlapTitle: '班次時間重疊',
    overlapMessage: '以下班次與即將儲存的班次時間重疊：',
    overlapSaveAnyway: '仍然儲存',
    overlapCancel: '返回修改',
  },
  en: {
    appTitle: 'Shifts Tracker',
    importCSV: 'Import CSV',
    exportCSV: 'Export CSV',
    settings: 'Settings',
    filterAll: 'All',
    filterWeek: 'Week',
    filterMonth: 'Month',
    filterYear: 'Year',
    noRecords: 'No shift records found',
    edit: 'Edit',
    duplicate: 'Duplicate',
    delete: 'Delete',
    notes: 'Notes',
    total: 'Total',
    annualLeave: 'Annual Leave',
    sickLeave: 'Sick Leave',
    overtime: 'Overtime',
    periodEarned: 'Earned',
    periodUsed: 'Used',
    leaveBalance: 'Leave Balance',
    estimateOnly: 'Estimates only',
    addShift: 'Add Shift',
    editShift: 'Edit Shift',
    date: 'Date',
    days: 'days',
    addDay: 'Add Day',
    addDayHint: 'Tip: You can add multiple days with the same shift at once',
    startTime: 'Start Time',
    endTime: 'End Time',
    breakTime: 'Break (minutes, unpaid)',
    dailyPaidHours: 'Daily Paid Hours',
    earnedLeave: 'Earned Leave',
    hourlyWage: 'Hourly Wage',
    notesOptional: 'Notes (Optional)',
    notesPlaceholder: 'Enter notes...',
    annualLeaveLabel: 'Annual Leave',
    annualLeaveDesc: 'Mark this shift as using annual leave',
    useHours: 'Hours Used',
    default: 'Default',
    settingsTitle: 'Settings',
    defaultWage: 'Default Hourly Wage',
    currency: 'Currency',
    defaultBreak: 'Default Break (minutes)',
    theme: 'Theme',
    themeLight: 'Light',
    themeDark: 'Dark',
    themeColor02: 'Color02 (Gundam)',
    language: 'Language',
    ukTax: 'UK Tax Estimates',
    ukTaxDesc: 'Auto estimate Tax, N.I., Pension',
    saveSettings: 'Save Settings',
    importSuccess: 'Successfully imported {count} shift records!',
    importError: 'Failed to parse CSV file. Please check the format.',
    clickDayHint: 'Click on a day to quickly add shifts in the same week',
    weekDays: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'],
    weekStartsOn: 'First Day of Week',
    weekStartsOnOptions: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'],
    listView: 'List',
    calendarView: 'Calendar',
    hours: 'hours',
    addPhoto: 'Add Photo',
    takePhoto: 'Take Photo',
    chooseFromGallery: 'Choose from Gallery',
    removePhoto: 'Remove Photo',
    photo: 'Photo',
    timer: 'Timer',
    breakTimer: 'Break Timer',
    startTimer: 'Start',
    pauseTimer: 'Pause',
    resumeTimer: 'Resume',
    resetTimer: 'Reset',
    paused: 'Paused',
    timerEnded: 'Break Ended',
    timerEndedDesc: 'Time to work!',
    timerWarning: 'Get Ready',
    timerWarningDesc: 'Break ends in 20 seconds',
    timerPausedDesc: 'Timer paused',
    minutes: 'minutes',
    min: 'min',
    sec: 'sec',
    reminders: 'Reminders',
    remindersDesc: 'Remind before shift',
    reminder1h: '1 hour before',
    reminder30m: '30 min before',
    hasReminders: 'Reminders set',
    jobs: 'Jobs',
    addJob: 'Add Job',
    editJob: 'Edit Job',
    jobName: 'Job Name',
    jobNamePlaceholder: 'Enter job name...',
    noJob: 'Unassigned',
    assignJob: 'Assign Job',
    allJobs: 'All Jobs',
    defaultJob: 'Default Job',
    setAsDefault: 'Set as Default',
    deleteJob: 'Delete Job',
    deleteJobConfirm: 'Are you sure you want to delete this job?',
    jobWage: 'Hourly Wage',
    jobBreak: 'Default Break (minutes)',
    noJobsYet: 'No jobs added yet',
    jobColor: 'Color',
    groupBy: 'Group by Job',
    liveEarning: 'Earning Now',
    excludeBreak: 'Exclude Break',
    timeLeft: 'Left',
    projectedTotal: 'Projected',
    importConflict: '{count} duplicate shift(s) found (same start & end time). Choose:',
    importConflictSkip: 'Skip Duplicates',
    importConflictAddAll: 'Add All',
    importConflictOverwrite: 'Overwrite',
    // Goal Tracker
    monthlyGoal: 'Monthly Goal',
    goalHours: 'Hours Goal',
    goalEarnings: 'Earnings Goal',
    goalProgress: 'This Month\'s Progress',
    goalHit: '🎉 Goal Reached!',
    goalNotSet: 'No goal set',
    setGoal: 'Set Goal',
    hoursGoal: 'Hours',
    earningsGoal: 'Earnings',
    // Monthly Report
    monthlyReport: 'Monthly Report',
    generateReport: 'Generate Report',
    reportTitle: 'Pay Slip',
    reportPeriod: 'Period',
    reportTotalHours: 'Total Hours',
    reportTotalEarnings: 'Total Earnings',
    reportRegularHours: 'Regular Hours',
    reportOvertimeHours: 'Overtime',
    reportAnnualLeave: 'Annual Leave',
    reportSickLeave: 'Sick Leave',
    reportShiftCount: 'Shifts',
    reportAvgPerShift: 'Avg/Shift',
    shareReport: 'Share Report',
    closeReport: 'Close',
    // Recurring Shifts
    recurring: 'Recurring',
    recurringNone: 'Does not repeat',
    recurringDaily: 'Daily',
    recurringWeekly: 'Weekly',
    recurringCustom: 'Custom Dates',
    recurringDays: 'Select Days',
    recurringUntil: 'Repeat Until',
    recurringPreview: 'Preview Shifts',
    recurringCount: '{count} shifts will be created',
    recurringSelectDays: 'Please select at least one day',
    // Paid Breaks
    paidBreaks: 'Paid Breaks',
    addPaidBreak: '+ Add Coffee Break',
    paidBreakDuration: 'Duration (minutes)',
    paidBreakCount: 'Count',
    paidBreakReminder: 'Start/End Reminder',
    paidBreakStart: '☕ Coffee Break',
    paidBreakReturn: '⏰ Back to Work',
    paidBreakLabel: 'Paid Break ×{count} ({min} min each)',
    paidBreakTotal: 'Total Paid Break',
    removePaidBreak: 'Remove',
    coffeeBreak: '☕ Coffee Break',
    startCoffeeBreak: 'Start {min} min Break',
    noPaidBreak: 'No paid breaks set',
    // Overlap
    overlapTitle: 'Shift Time Overlap',
    overlapMessage: 'The following existing shifts overlap with your new shift:',
    overlapSaveAnyway: 'Save Anyway',
    overlapCancel: 'Go Back',
  }
};

export function useTranslation(lang: 'zh' | 'en' = 'zh') {
  return translations[lang] || translations.zh;
}
