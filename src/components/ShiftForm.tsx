...
export default function ShiftForm({ shift, settings, onSave, onCancel, jobs = [] }: Props) {
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--color-bg-light)] text-slate-900">
      {/* Header */}
      <header className="flex items-center justify-between px-6 pt-4 pb-2 bg-white border-b border-slate-100">
        <div className="flex items-center gap-3">
          <button onClick={handleCancel} className="p-2 hover:bg-slate-50 rounded-full transition-colors">
            <span className="material-symbols-outlined text-slate-600">close</span>
          </button>
          <h1 className="text-2xl font-bold text-slate-800">{isEditing ? t.editShift : t.addShift}</h1>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6 custom-scrollbar">
        ...
        <div>
          ...
          <div className="relative">
            <select 
              ...
              className="w-full appearance-none bg-slate-50 border-none rounded-xl px-4 py-4 text-slate-800 focus:ring-2 focus:ring-[var(--color-primary)]/20 transition-all font-medium"
            >
        ...
        {/* Date Picker Section */}
        <div>
          <label className="block text-sm font-semibold text-slate-500 mb-2 ml-1 uppercase tracking-wider">Shift Date</label>
          <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-100">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="font-bold text-slate-800 text-lg">{format(currentMonth, 'MMMM yyyy')}</span>
              <button 
                onClick={() => setCurrentMonth(addDays(currentMonth, 30))} 
                className="p-2 hover:bg-slate-50 rounded-full transition-colors"
              >
          ...
          <div>
            ...
            <div className="bg-slate-50 rounded-xl p-2 flex items-center justify-center">
              <input
                ...
                className="bg-transparent border-none text-center text-xl font-bold text-slate-800 outline-none w-full"
              />
            </div>
          </div>
          <div>
            <label className="block text-sm font-semibold text-slate-500 mb-2 ml-1 uppercase tracking-wider">End Time</label>
            <div className="bg-slate-50 rounded-xl p-2 flex items-center justify-center">
              <input
                type="time"
                value={timeEnd}
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full p-4 bg-slate-50 border-none rounded-xl text-slate-800 font-medium resize-none"
          />
        </div>
            ...
            <div className="relative">
              ...
              {showPhotoOptions && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden z-10 animate-scale-in">
                  ...
                  >
                    <span className="material-symbols-outlined">camera_alt</span> {t.takePhoto}
                  </button>
                  <button
                    onClick={handleChooseFromGallery}
                    className="w-full px-4 py-3.5 text-left text-sm font-medium flex items-center gap-3 bg-slate-50 text-slate-700"
                  >
                    <span className="material-symbols-outlined">photo_library</span> {t.chooseFromGallery}
                  </button>
                </div>
              )}
            </div>
